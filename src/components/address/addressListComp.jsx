...
import { Card, CardContent, Typography, Button, Radio, Dialog, DialogActions, DialogContent, DialogTitle, TextField, ToggleButton, ToggleButtonGroup } from '@mui/material';
...
const UserAddressList = ({ addresses, onUpdateAddresses,onSelectAddress }) => {
    ...
    const handleSaveAddress = () => {
        ...
        onUpdateAddresses(currentAddress);
        setIsEditMode(false);
        setIsAddMode(false);
    };
    ...
    return (
        <div style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {addresses.map((address) => (
                <Card
                    ...
                    style={{
                        ...
                        padding: '16px',
                        cursor: 'pointer',
                        border: selectedAddressId === address.id ? `2px solid ${theme.palette.primary.main}` : '1px solid #e5e7eb',
                ...
                >
                    ...
                    />
                    <CardContent style={{ flexGrow: 1, marginLeft: '8px', minWidth: '250px' }}>
                        ...
                        <Typography variant="body2">
                            {address.address.door}, {address.address.building}, {address.address.street}, {address.address.city}, {address.address.state}
                        </Typography>
                        <Typography variant="body2" color="textSecondary">
            ...
            <Dialog open={isEditMode || isAddMode} onClose={() => { setIsEditMode(false); setIsAddMode(false); }}>
                <DialogTitle>{isEditMode ? 'Update Delivery Address' : 'Add Delivery Address'}</DialogTitle>
                <DialogContent>
                    <TextField
                        label="Name*"
                    ...
                    <TextField
                        ...
                        margin="normal"
                    />
                            <MapPicker address={currentAddress.address} setAddress={setCurrentAddress} />
                    <TextField
                        label="Building*"
                    ...
                    {/* Tag Selection */}
                    <Typography variant="body1" style={{ marginTop: '16px' }}>Tag</Typography>
                    <ToggleButtonGroup
                    ...
                    >
                        <ToggleButton 
                            value="Home" 
                            style={{
                                backgroundColor: currentAddress.address.tag === 'Home' ? theme.palette.primary.main : undefined,
                                color: currentAddress.address.tag === 'Home' ? 'white' : undefined,
                            }}
                        >
                            Home
                        </ToggleButton>
                        <ToggleButton 
                            value="Office" 
                            style={{
                                backgroundColor: currentAddress.address.tag === 'Office' ? theme.palette.primary.main : undefined,
                                color: currentAddress.address.tag === 'Office' ? 'white' : undefined,
                            }}
                        >
                            Office
                        </ToggleButton>
                        <ToggleButton 
                            value="Other" 
                            style={{
                                backgroundColor: currentAddress.address.tag === 'Other' ? theme.palette.primary.main : undefined,
                                color: currentAddress.address.tag === 'Other' ? 'white' : undefined,
                            }}
                        >
                            Other
                        </ToggleButton>
                ...
                <DialogActions>
                    <Button onClick={() => { setIsEditMode(false); setIsAddMode(false); }}>Cancel</Button>
