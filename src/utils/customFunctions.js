...
export const getAmount = (
    ...
    console.log("function to calculate",amount, currency_symbol_direction, currency_symbol, digitAfterDecimalPoint);
...
export const getSubTotalPrice = (cartList) => {
    ...
    console.log("cartList12345",cartList);
    let ad = cartList.reduce(
        (total, product) =>product?.item?.product?.price?.value *product.item?.quantity?.count +total,
        0
    )
    return ad
}
...
export const cartItemsTotalAmount = (cartList) => {
    ...
    if (cartList?.length > 0) {
        cartList?.forEach((item) => {
            console.log("cartlist eac item price",item);
